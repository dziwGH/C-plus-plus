...
string mainItemFunct(string $mainItem, int $choice);
...
int main()
{
    ...
    char size, correct = 'y';
    double subtotal = 0, taxTotal = 0, grandTotal = 0, cost = 0;
    ...
    // Function call to print the menu
    menuFunct();
   ...
   // Ask for user input
    ...
    do {
        ...
        cin >> num_OF_orders;
        cout << endl; // endl so that it looks pretty :D
        if (!cin) 
        {
            ...
            inputFailure = true; // boole variable to test for input failure
        }
    ...
    for (i = 1; i <= num_OF_orders; i++) // main loop
    {
        ...
        do { // loop to run no matter what
            ...
            do { // loop for menu number while the correct number(s) are entered
        ...
        // asking the user for the size of the meal they would like
            ...
            // Function calls
            mainItemFunct(mainItem, choice); // Assigns the mainItem variable, choice is a user input variable the determines the mainItem
            

            
            cout << "You ordered a " << sizeName << " " << mainItemFunct(mainItem, choice) << "." << endl << endl;
