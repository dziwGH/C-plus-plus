...
int main()
{
    ...
    fout.open("grades_formatted.txt");
    ...
    // Read Data from File - Person 2
    getline(fin, name);
    fin >> test1 >> test2 >> test3;
    fin.ignore();

    // Data Calculation - Person 2
    testAvg = (test1 + test2 + test3) / 3;

    // Output to Files - Person 2
    fout << setprecision(0) << fixed << noshowpoint;
    fout << setw(COL1) << left << name << setw(COL2) << right << test1 << setw(COL3) << right << test2 << setw(COL4) << right << test3 << setw(COL5) << right << setprecision(2) << fixed << showpoint << testAvg << endl;

    // Read Data from File - Person 3
    getline(fin, name);
    fin >> test1 >> test2 >> test3;
    fin.ignore();

    // Data Calculation - Person 3
    testAvg = (test1 + test2 + test3) / 3;

    // Output to Files - Person 3
    fout << setprecision(0) << fixed << noshowpoint;
    fout << setw(COL1) << left << name << setw(COL2) << right << test1 << setw(COL3) << right << test2 << setw(COL4) << right << test3 << setw(COL5) << right << setprecision(2) << fixed << showpoint << testAvg;
